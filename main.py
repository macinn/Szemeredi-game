...
def settings_screen() -> dict:
    ...
    algo_names: list[str] = list(registry.keys())
    if not algo_names:
        algo_names = ["random"]
    input_boxes: dict[str, dict] = {
    ...
    start_button: pygame.Rect = pygame.Rect(250, 450, 120, 50)
    exit_button: pygame.Rect = pygame.Rect(430, 450, 120, 50)
    active_box: str | None = None
    color_inactive = pygame.Color('lightskyblue3')
    color_active = pygame.Color('dodgerblue2')
    error_msg: str = ""
    error_timer: int = 0
    # Scroll parameters 
    visible_algo_count: int = 5
    ...
    while running:
        for event in pygame.event.get():
                ...
                if algo_box["rect"].collidepoint(event.pos):
                    ...
                    if first_box["open"]:
                        for i, option in enumerate(first_box["options"]):
                            opt_rect = pygame.Rect(
                                first_box["rect"].x,
                                first_box["rect"].y + first_box["rect"].h + i * option_height,
                                first_box["rect"].w,
                                option_height
                            )
                            if opt_rect.collidepoint(event.pos):
            ...
            if event.type == pygame.MOUSEWHEEL:
                if algo_box["open"]:
                    max_offset = max(0, len(algo_box["options"]) * option_height - visible_algo_count * option_height)
                    algo_box["scroll_offset"] = max(0, min(max_offset, algo_box["scroll_offset"] - event.y * option_height))
